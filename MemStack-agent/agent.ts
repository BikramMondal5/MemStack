/** MemStack-agent — LangChain.js Agent on the Zynd network.
 *
...
function buildLangchainAgent(zyndAgent: ZyndAIAgent, registryUrl: string) {
  ...
  const prompt = ChatPromptTemplate.fromMessages([
    [
      "system",
      "You are MemStack-agent, a helpful AI assistant. " +
...
async function main() {
  const agentConfig = AgentConfigSchema.parse({
    name: _config.name ?? "MemStack-agent",
    description: _config.description ?? "MemStack-agent — a LangChain.js agent on the Zynd network.",
  ...
  console.log(`\nMemStack-agent is running (LangChain.js)`);
